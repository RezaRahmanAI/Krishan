...
{
    public class RawProductDeductController : CoreController
    {
        ...
        public RawProductDeductController(IErrorService errorService,
        ...
        //    var customPO = _purchaseOrderService.GetAllRawStockDeductOrderAsync();
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var customPO = _purchaseOrderService.GetAllRawStockDeductOrderAsync(ViewBag.FromDate, ViewBag.ToDate);
            var vmPO = _mapper.Map<IEnumerable<Tuple<int, string, DateTime, string, string, string, EnumPurchaseType>>,
                IEnumerable<GetPurchaseOrderViewModel>>(await customPO);
            return View(vmPO);
        ...
        private ActionResult ReturnCreateViewWithTempData()
        {
            ...
            {
                string chllnNo = "RSD-" + _miscellaneousService.GetUniqueKey(x => x.POrderID);
        ...
        {
            var FirstSupplier = _supplierService.GetAllIQueryable().FirstOrDefault();

            var POrderDetails = CreateFIFODetails(purchaseOrder);
            #region POrder
            purchaseOrder.PurchaseOrder.Status = ((int)EnumPurchaseType.RawStockDeduct).ToString();
            ...
            purchaseOrder.PurchaseOrder.SupplierId = FirstSupplier.SupplierID.ToString();
            ...
            var Result = _purchaseOrderService.AddRawStockOutOrderUsingSP(dtPurchaseOrder, dtPurchaseOrderDetail, dtPOProductDetail);

            if (Result.Item1)
            {
                AddToastMessage("", "Raw Material Product Deduct Successfull.", ToastType.Success);
                TempData["POrderID"] = Result.Item2;
                TempData["IsChallanReadyByID"] = true;
            }
            else
                AddToastMessage("", "Raw Material Product Deduct Failed.", ToastType.Error);
