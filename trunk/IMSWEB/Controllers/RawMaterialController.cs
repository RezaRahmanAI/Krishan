...
namespace IMSWEB.Controllers
{
    [Authorize]
    [RoutePrefix("product")]
    public class RawMaterialController : CoreController
    {
        ...
        public RawMaterialController(IErrorService errorService,
            IProductService productService, IMiscellaneousService<Product> miscellaneousService, IMapper mapper,
            IProductUnitTypeService ProductUnitTypeService, IPurchaseOrderService PurchaseOrderService, IDiaSizeService DiaSizeService,ICategoryService categoryService,
            ICompanyService companyService, ISizeService sizeService)
            : base(errorService)
        {
        ...
        public ActionResult Index(int? Page)
        {
            ...
            var customProductAsync = _productService.GetAllProductIQueryableIndex(EnumProductStockType.Raw_Materials);
        ...
        public ActionResult Index(FormCollection formCollection)
        {
            ...
            if (!string.IsNullOrEmpty(formCollection["ProductName"]))
            {
                ...
                customProductAsync = _productService.GetAllProductIQueryableIndex(EnumProductStockType.Raw_Materials).Where(i => i.ProductName.Contains(productName));
            }
            var vmodel = _mapper.Map<IQueryable<ProductWisePurchaseModel>, List<GetProductViewModel>>(customProductAsync);
            var pagelist = vmodel.ToPagedList(Pages, PageSize);
            return View(pagelist);
...

        {
            ...
            var category = _categoryService.GetAllIQueryable().FirstOrDefault();
            var size = _sizeService.GetAllIQueryable().FirstOrDefault();
            var compnay = _companyService.GetAllIQueryable().FirstOrDefault();


            //var DiaSize = _DiaSizeService.GetAll().FirstOrDefault();
            return View(PopulateDropdown(new CreateProductViewModel { Code = code, UnitType = (UnitType == null ? 0 : UnitType.ProUnitTypeID), ProductType = EnumProductType.NoBarcode, CategoryId = Convert.ToString(category.CategoryID), CompanyId = Convert.ToString(compnay.CompanyID), SizeID = size.SizeID }));
        }
        ...
        HttpPostedFileBase picture, string returnUrl)
        {
            ...
            if (newProduct != null)
            {
                ...
                product.ProductStockType = EnumProductStockType.Raw_Materials;
            ...
            HttpPostedFileBase picture, string returnUrl)
        {
            ...
            if (newProduct != null)
            {
                ...
                //existingProduct.DiaSizeID = newProduct.DiaSizeID;
                ...
                existingProduct.ProductStockType = EnumProductStockType.Raw_Materials;
        ...
        private void CheckAndAddModelError(CreateProductViewModel newProduct, FormCollection formCollection)
        {
            ...
            //if (string.IsNullOrEmpty(formCollection["DiaSizeID"]))
            //    ModelState.AddModelError("DiaSizeID", "Dia. Size is required");
